...
namespace FacturacionVERIFACTU.API.Data.Services
{
    ...
    public class AlbaranService : IAlbaranService
    {
            ...
            foreach(var lineaDto in dto.Lineas)
            {
                if(lineaDto.Id.HasValue && lineaDto.Id.Value > 0)
                {
                    ...
                    if(lineaExistente!= null)
                    {
                        ...
                        lineaExistente.IVA = lineaDto.IVA;
        ...
        {
            albaran.BaseImponible = albaran.Lineas.Sum(l => l.BaseImponible);
            albaran.TotalIVA = albaran.Lineas.Sum(l => l.ImporteIva);
            albaran.Total = albaran.Lineas.Sum(l => l.Importe);
        }
